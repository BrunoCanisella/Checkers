...
class Jogo:
	...
	def movimento_obrigatorio(self, localizacao_cedula):
		...
		self.turno % 2 == index:
				if l > 0:
					#movimento Obrigatorio para a Direita peca branca
					if c < 7:
			...
			#movimento Obrigatorio  para a esquerda peca branca
					if c > 0:
						if self.matriz_jogadores[l - 1][c - 1].lower() not in array:
							l_x = l - 1
							l_c = c - 1
							...
							if l_x - 1 >= 0 and l_c - 1 >= 0:
								if self.matriz_jogadores[l_x - 1][l_c - 1] == '-':
									...
									posicao_cedula_pulada.append((l_x, l_c))
				if l < 7:
					#movimento Obrigatorio para a direta peca preta
					if c < 7:
						if self.matriz_jogadores[l + 1][c + 1].lower() not in array:
							l_x = l + 1
							l_c = c + 1
							...
							if l_x + 1 <= 7 and l_c + 1 <= 7:
								if self.matriz_jogadores[l_x + 1][l_c + 1] == '-':
									obrigatorios.append([l_x + 1, l_c + 1])
									posicao_cedula_pulada.append((l_x, l_c))
					#movimento Obrigatorio para a esquerda peca preta 
					if c > 0:
						if self.matriz_jogadores[l + 1][c - 1].lower() not in array:
							l_x = l + 1
							l_c = c - 1
				...
				while True:
					if conta_linha + 1 > 7 or conta_coluna + 1 > 7: break
						...
						if self.matriz_jogadores[conta_linha + 1][conta_coluna + 1] not in array:
							l_x = conta_linha + 1
							l_c = conta_coluna + 1
#verifica se tem mais de uma jogada obrigatoria se linha ou coluna for menor ou igual a 7
							if l_x + 1 <= 7 and l_c + 1 <= 7:
								if self.matriz_jogadores[l_x + 1][l_c + 1] == '-':
									posicao_cedula_pulada.append((l_x, l_c))
									while True:
				...
				while True:
					if conta_linha + 1 > 7 or conta_coluna - 1 < 0: break 
						...
						if self.matriz_jogadores[conta_linha + 1][conta_coluna - 1] not in array:
							l_x = conta_linha + 1
							l_c = conta_coluna - 1
#movimentos obrigatorios quando linha for menor ou igual a 7 e coluna maior ou igual a 0
							if l_x + 1 <= 7 and l_c - 1 >= 0:
								if self.matriz_jogadores[l_x + 1][l_c - 1] == '-':
									posicao_cedula_pulada.append((l_x, l_c))
									while True:
		...
		if movimentos == []:
			...
			if self.matriz_jogadores[linha_atual][coluna_atual].islower():
				if self.matriz_jogadores[linha_atual][coluna_atual] == 'o':
					if linha_atual > 0:
						if coluna_atual < 7: #se coluna for maior que 0 permite mover peca para uma coluna  que seja menor que 0 se a linha for menor que 7
							if self.matriz_jogadores[linha_atual - 1][coluna_atual - 1] == '-':
								if self.matriz_jogadores[linha_atual - 1][coluna_atual + 1] == '-':
									movimentos.append([linha_atual - 1, coluna_atual + 1])
						if coluna_atual > 0: #se coluna for maior que 0 permite mover peca para uma coluna  que seja menor que 0 se a linha for maior que 0
							if self.matriz_jogadores[linha_atual - 1][coluna_atual - 1] == '-':
								movimentos.append([linha_atual - 1, coluna_atual - 1])
				
				elif self.matriz_jogadores[linha_atual][coluna_atual] == 'x':
					if linha_atual < 7:
						if coluna_atual < 7: #se coluna menor que 7 permite mover peca para uma linha  que seja menor que 7 se a linha for menor que 7
							if self.matriz_jogadores[linha_atual + 1][coluna_atual + 1] == '-':
								movimentos.append([linha_atual + 1, coluna_atual + 1])
						if coluna_atual > 0: #se coluna maior que 0 e linha menor que 7 permite mover peça para uma linha que seja maior que 0
							if self.matriz_jogadores[linha_atual + 1][coluna_atual - 1] == '-':
								movimentos.append([linha_atual + 1, coluna_atual - 1])
				...
				while True:
					if conta_linha - 1 < 0 or conta_coluna - 1 < 0: break # se Coluna  e linha for menor que 0 na matriz não permite o  movimento 
					else:
				...
				conta_coluna = coluna_atual
				while True:
					if conta_linha - 1 < 0 or conta_coluna + 1 > 7: break # se linha menor que 0 e coluna maior que 7 não permite
					else:
						if self.matriz_jogadores[conta_linha - 1][conta_coluna + 1] == '-':
							movimentos.append([conta_linha - 1, conta_coluna + 1])
						else: break
				...
				conta_coluna = coluna_atual
				while True:
					if conta_linha + 1 > 7 or conta_coluna + 1 > 7: break # se linha e coluna for maior que 7 ele não permite movimentar
					else:
						if self.matriz_jogadores[conta_linha + 1][conta_coluna + 1] == '-':
							movimentos.append([conta_linha + 1, conta_coluna + 1])
						else: break
				...
				conta_coluna = coluna_atual
				while True:
					if conta_linha + 1 > 7 or conta_coluna - 1 < 0: break #se linha for maior que 7 e coluna menor que 0 ele não permite movimento
					else:
						if self.matriz_jogadores[conta_linha + 1][conta_coluna - 1] == '-':
							movimentos.append([conta_linha + 1, conta_coluna - 1])
						else: break
	...
	def jogar(self, jogador, localizacao_cedula, linha_destino, coluna_destino, pulo):
		...
		if (jogador == 'x' and linha_destino == 7) or (jogador == 'o' and linha_destino == 0): #transforma a peça em Dama andando pelas casas
			if not self.pulando:
		...
		if (jogador == 'x' and linha_destino == 7) or (jogador == 'o' and linha_destino == 0): #transforma a peça em Dama comendo peça
			if  self.pulando:
				self.matriz_jogadores[linha_destino][coluna_destino] = char.upper()
...
def creditos():
	...
	while not sair:
		...
		rect_texto.center = ((LARGURA / 2), ALTURA / 1.78)
		display.blit(surface_texto, rect_texto)

		surface_texto, rect_texto = text_objects("Versao Python: 2.10", fonte, VERMELHO_CLARO)
...
def regras():
	...
	while not sair:
		...
		info4 = fonte.render('Quando a peça atinge a oitava linha do tabuleiro ela vira dama.', False, (BRANCO))
		...
		info8 = fonte.render('A peça sem ser dama captura só para frente', False, (BRANCO))
		info9 = fonte.render('Vence quem ficar com peças sobrando no tabuleiro', False, (BRANCO))
		info10 = fonte.render('O empate vem quando se passa mais de 10 turnos e sobram uma peça de cada oponente', False, (BRANCO))
		
		game1 = fonte.render('Durante o jogo, ao clicar em uma peca, sera exibido em verde os movimentos', False, (BRANCO))
		game2 = fonte.render('possiveis da mesma. Se nada acontecer ao clicar em uma peca, significa que', False, (BRANCO))
