...
class Jogo:
	...
	def movimento_obrigatorio(self, localizacao_cedula):
				...
				if l > 0:
					if c < 7:#movimento Obrigatorio para a Direita peca branca 
						if self.matriz_jogadores[l - 1][c + 1].lower() not in array and jogador=='o':
							l_x = l - 1
							l_c = c + 1
							if l_x - 1 >= 0 and l_c + 1 <= 7:#comer peça direita 
								if self.matriz_jogadores[l_x - 1][l_c + 1] == '-':
									obrigatorios.append([l_x - 1, l_c + 1])
									posicao_cedula_pulada.append((l_x, l_c))
					if c > 0:#movimento peca branca esquerda							#and jogador=='peca' bloqueia comer para traz
						if self.matriz_jogadores[l - 1][c - 1].lower() not in array and jogador=='o':
							l_x = l - 1
							l_c = c - 1
							if l_x - 1 >= 0 and l_c - 1 >= 0:#comer peça esquerda
								if self.matriz_jogadores[l_x - 1][l_c - 1] == '-':
									obrigatorios.append([l_x - 1, l_c - 1])
									posicao_cedula_pulada.append((l_x, l_c))
		...
		self.turno % 2 == index:
			...
			if not self.pulando and (jogador.lower() == 'x' and l !=7 )  or (jogador.lower() == 'o' and l !=0)  : #foi colocado diferente ou igual a 8 e diferente igual a -1 para poder comer se virar dama a partir da casa de dama.
				conta_linha = l
				conta_coluna = c
				while True:
				...
				while True:
					if conta_linha + 1 > 7 or conta_coluna + 1 > 7: break
						...
						if self.matriz_jogadores[conta_linha + 1][conta_coluna + 1] not in array:
							l_x = conta_linha + 1
							l_c = conta_coluna + 1
							if l_x + 1 <= 7 and l_c + 1 <= 7:#verifica se tem mais de uma jogada obrigatoria se linha ou coluna for menor ou igual a 7
								if self.matriz_jogadores[l_x + 1][l_c + 1] == '-':
									posicao_cedula_pulada.append((l_x, l_c))
									while True:
